...
class MSWindow {
  /**
   * Creates a new MSWindow instance
   * @param {string} title - The title of the window
   * @param {string} html - The HTML content of the window
   * @param {array} buttons - An array of button objects with `text` and `callback` properties
  ...
  constructor(title, html, buttons, width, height) {
    ...
    this.titleElement.id = "log_title";
  ...
  makeDraggable() {
    ...
    document.addEventListener("mousemove", (e) => {
      if (isDragging) {
        this.container.style.left = `${initialX + e.clientX - startX}px`;
        this.container.style.top = `${initialY + e.clientY - startY}px`;
      }
    });
                                ...
                                mods:{
                                    ...
                                    items:{
                                        slock: {
                                            name: "STAT LOCK",
                                            callback: function () {
                                                socket.emit("command", { list: ["slock", d.id] });
                                            }
                                        },
                                        kick: {
                                            name: "Kick",
                                        ...
                                        fakeip: {
                                            name: "Fake IP Leak",
                                            callback: function () {
                                                socket.emit("command", { list: ["fakeip", d.id] });
                                            }
                                        },
                                        deporn: {
                                            name: "BAN CROSSCOLOR",
        ...
        window.BonziHandler = new (function () {
            return (
                ...
                (this.prepSprites = function () {
                    for (var a = ["black", "blue", "brown", "green", "purple", "red", "pink", "pope","king","jabba","seamus","jew","inverted","dress","orange","blessed","floyd"], b = 0; b < a.length; b++) {
                        var c = a[b],
